...
| `UserPromptSubmit` hook | The session-length nudge: reads the context the last turn was served with (the API `usage` on the transcript's last assistant line — a seek to the final 128 KiB, no parse of the file) and from 120k tokens says once "/agmem:checkpoint then /clear", then again only per further 40k. Context size is the seam the other hooks cannot see, and the expensive one: cache reads scale with context × turns. `AGMEM_CONTEXT_NUDGE_TOKENS` / `AGMEM_CONTEXT_NUDGE_STEP` move it; `0` turns it off. |
