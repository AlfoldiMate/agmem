...
//!
//! Ignored by default: the first run fetches the weights from Hugging Face
//! (~314 MB for EmbeddingGemma, ~36 MB for bge-small), which is not
//! something CI should do on every push (#119). Run deliberately with
//! `cargo test -p agmem-embed --test llama -- --ignored`. `AGMEM_MODEL`
//! picks the model where a test does not pin one; `AGMEM_ACCELERATOR`
//! (`auto|cpu|metal`) where it runs.

use agmem_embed::{Accelerator, Embedder, LlamaEmbedder, Model};

/// Cosine similarity; the backend L2-normalises, so this is just a dot
/// product, but spelling it out keeps the test honest if that ever changes.
...
/// The model `AGMEM_MODEL` names, else the default.
fn model_from_env() -> Model {
    match std::env::var("AGMEM_MODEL") {
        Ok(spelling) => Model::parse(&spelling)
            .unwrap_or_else(|| panic!("AGMEM_MODEL={spelling:?}: not a model spelling")),
        Err(_) => Model::default(),
    }
}

/// Where `AGMEM_ACCELERATOR` says, else `auto`.
fn accelerator_from_env() -> Accelerator {
    match std::env::var("AGMEM_ACCELERATOR") {
        Ok(spelling) => Accelerator::parse(&spelling)
            .unwrap_or_else(|| panic!("AGMEM_ACCELERATOR={spelling:?}: one of auto, cpu, metal")),
        Err(_) => Accelerator::Auto,
    }
}

/// Load `model` into a cache under the temp dir, not the user's model dir.
fn load(model: Model) -> LlamaEmbedder {
    let cache = std::env::temp_dir().join("agmem-model-cache");
    LlamaEmbedder::new(model, Some(cache), accelerator_from_env()).expect("load model")
}

/// Shortest-round-tripping decimal for each component, so f32 → text → f64
/// → f32 is exact both ways.
...
fn related_sentences_land_closer_than_unrelated_ones() {
    ...
    let spec = model.spec();
    assert_eq!(embedder.dim(), spec.dim);
    assert_eq!(embedder.model_id(), spec.id);
    ...
    assert!(vectors.iter().all(|vector| vector.len() == spec.dim));
    ...
    // The model's own bands must agree with the fixture on the easy half:
    // the tap is under the correction floor. The paraphrase is not asserted
    // against it — Gemma scores this pair 0.694 against a 0.70 floor whose
    // corrected-pair p5 is 0.68 (`docs/eval/embed-models.md`), so a single
    // pair says nothing the eval set has not already said.
    let bands = embedder.thresholds();
    eprintln!(
        "{}: paraphrase {related:.4}, unrelated {unrelated:.4}, correction floor {}",
        spec.id, bands.correction_floor
    );
    assert!(
        f64::from(unrelated) < bands.correction_floor,
        "{unrelated} should sit under {}'s correction floor {}",
        spec.id,
        bands.correction_floor
    );
...
/// file is where the claim lives. Pinned to bge-small because the claim is
/// about bge's query vectors.
///
/// Run with `cargo test -p agmem-embed --test llama -- --ignored
...
fn regenerate_knn_fixture() {
    ...
    let document = format!(
        "{{\n  \"model\": {},\n  \"dim\": {},\n  \
         \"rows\": [\n{}\n  ],\n  \"queries\": [\n{}\n  ]\n}}\n",
        serde_json::to_string(spec.id).expect("encode model"),
        spec.dim,
        entries.join(",\n"),
        probes.join(",\n")
    );
...
/// through a `RecordedEmbedder` so its numbers carry real model semantics
/// while staying bit-stable and offline. This is the recorder: it walks the
/// eval scenario fixtures, embeds every passage and query they use, and
/// commits the result as one JSON map. Run it after any edit to a scenario
/// file — an unrecorded string makes `RecordedEmbedder` panic, deliberately,
/// rather than fall back to a zero vector that would read as a scoring
/// regression.
///
...
/// rather than rewriting the corpus's vectors.
///
/// The default model writes the committed fixtures; `AGMEM_MODEL` records
/// the same texts with another model into a gitignored sibling directory
/// the eval reads through `AGMEM_EVAL_VECTORS_DIR`
/// (`docs/eval/embed-models.md`).
///
/// Run with `cargo test -p agmem-embed --test llama -- --ignored
...
fn regenerate_eval_vectors() {
    ...
    let model = model_from_env();
    let spec = model.spec();
    let out_dir = if model == Model::default() {
        eval_dir.clone()
    } else {
        eval_dir.join("candidates").join(spec.id)
    };
    let embedder = load(model);
